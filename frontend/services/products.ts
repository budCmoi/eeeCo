...
function filterLocalProducts(products: Product[], filters: ProductFilters = {}) {
  ...
  if (filters.delivery) {
    nextProducts = nextProducts.filter((p) => (p.deliveryDays ?? 5) <= filters.delivery!);
  }
...
export async function fetchProducts(filters: ProductFilters = {}) {
  try {
    ...
    const { data } = await api.get<Product[] | { items: Product[] }>('/products', { params });
...
export async function fetchRelatedProducts(product: Product) {
  ...
  return products.filter((candidate) => candidate.slug !== product.slug).slice(0, 4);
}